...
class DataSet():
    def __init__(self, path, key='__eou__'):
        self.key = key
        self.questions = []
        self.answers = []
    ...
    def __call__(self, remove_emoticons=False):
        for line in self.lines:
            list_text = line.split(str(self.key))
            self.questions.append(self.clear_data(list_text[0], remove_emoticons))
            self.answers.append(self.clear_data(list_text[1], remove_emoticons))

        return [self.questions, self.answers]

    def clear_data(self, string, remove_emoticons=False):
        string.lower()
        string = re.sub(r'[@_!#$%^&*()<>?/\|}{~:,.]', "", string)
        string = self.remove_emoji(string)
        if remove_emoticons:
            string = self.remove_emoticons(string)
        return string.strip()
